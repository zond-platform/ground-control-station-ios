...
fileprivate let defaultVertexRadius: CLLocationDistance = 50

class MissionPolygon : MKPolygon {
    // Stored properties
    var dragIndex: Int?
    var vertexRadius: CLLocationDistance = defaultVertexRadius
    ...
    // Observer properties
    var gridDistance: CGFloat = CGFloat(MissionParameterId.meanderStep.defaultValue) {
        didSet {
            updated?()
        }
    }
    var gridAngle: CGFloat = CGFloat(MissionParameterId.meanderAngle.defaultValue) {
        didSet {
            updated?()
        }
    }
